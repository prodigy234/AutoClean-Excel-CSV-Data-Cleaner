...
if uploaded_file:
    try:
        ...
        if st.button("✨ Clean Data"):
            ...
            st.write(f"✅ **Duplicates Removed:** {df.duplicated().sum()}")
            st.write(f"✅ **Missing Values Filled:** {df.isnull().sum().sum()}")
            st.write(f"✅ **Special Characters Removed:** {sum(cleaned_df[col].str.contains(r'[^a-zA-Z0-9 ]').sum() for col in cleaned_df.select_dtypes(include=['object']).columns)}")
